...
//Enables testing of extra credit index function
#define TEST_INDEXING 0
...
// INDEX TESTING FUNCTIONS BEGIN
#if TEST_INDEXING
template <typename T>
void indexingTest(int count) {
  using namespace std::chrono;
  T m = ascendingInsert<T>(count,false);
  
  TimePoint start = system_clock::now(), end;
  std::default_random_engine generator;
  std::uniform_int_distribution<int> distribution(0,m.size() - 1);
  for (int j = 0; j < count; j++) {
    int ind = distribution(generator);
    int f_i = m.index(ind);
    //std::cout << "Found index " << ind << " to be " << f_i << std::endl;
  }

  end = system_clock::now();
  Milli elapsed = end - start;
  
  std::cout << "Indexing " << count << " elements in a map of size " << count << " took " << elapsed.count() << " milliseconds time per iteration was " << elapsed.count()/double(count)*1e6 << " nanoseconds" << std::endl;
}

template <typename T>
void indexingTest_ON(int count) {
  using namespace std::chrono;
  T m = ascendingInsert<T>(count,false);
  ...
  std::cout << "Indexing [in O(n)] " << count << " elements in a map of size " << count << " took " << elapsed.count() << " milliseconds time per iteration was " << elapsed.count()/double(count)*1e6 << " nanoseconds" << std::endl;
}
#endif
// INDEX TESTING FUNCTIONS END
...
int main() {
  ...
  */
  ...
  #if TEST_INDEXING
  dispTestName("Indexing test", w);
  indexingTest<cs540::Map<int,int>>(10000);
  indexingTest_ON<cs540::Map<int,int>>(10000);
  indexingTest<cs540::Map<int,int>>(100000);
  indexingTest_ON<cs540::Map<int,int>>(100000);
  //indexingTest<cs540::Map<int,int>>(1000000);
  //indexingTest<cs540::Map<int,int>>(10000000);
  #endif 
